...
class Command:
    ...
    def get_command_line_list(self) -> List[str]:
        """
        Return a list of strings suitable for a command line command for a rippled
        server
        """
    ...
    def get_websocket_dict(self) -> dict:
        """
        Return a dictionary suitable for converting to json and sending to a rippled
    ...
    def to_cmd_obj(self) -> dict:
        """
        Return an object suitablee for use in a command (such as input to json.dumps or
        similar)
        """
        assert False
        return {}
...
class WalletPropose(Command):
    ...
    def get_command_line_list(self) -> List[str]:
        ...
        assert (
            not self.seed
            and not self.seed_hex
            and (not self.key_type or self.key_type == "secp256k1")
        )
        if self.passphrase:
...
class AccountLines(Command):
    ...
    def get_command_line_list(self) -> List[str]:
        ...
        assert (
            sum(
                x is None
                for x in [self.ledger_index, self.ledger_hash, self.limit, self.marker]
            )
            == 4
        )
        result = [self.cmd_name(), self.account.account_id]
...
class AccountTx(Command):
    def __init__(self, account: Account, *, limit: Optional[int] = None, marker=None):
        super().__init__()
        self.account = account
        self.limit = limit
        self.marker = marker
...
class BookOffers(Command):
    ...
    def get_command_line_list(self) -> List[str]:
        ...
        return [self.cmd_name(), self.taker_pays.cmd_str(), self.taker_gets.cmd_str()]
...
class Subscribe(SubscriptionCommand):
    ...
    def __init__(
        ...
        *,
        ...
        books: Optional[List[BookSubscription]] = None,  # taker_pays, taker_gets
    ...
    def to_cmd_obj(self) -> dict:
        ...
        if self.accounts_proposed is not None:
            d["accounts_proposed"] = [a.account_id for a in self.accounts_proposed]
