...
from slk.command import Command, LogLevel, ServerInfo, Stop, SubscriptionCommand
...
class RippleClient:
        ...
        for retry_count in range(0, max_retries + 1):
            try:
                ...
                eprint(
                    ...
                    f"retrying..{retry_count+1} of {max_retries}"
                )
                time.sleep(1)  # give process time to startup

    async def _send_websock_command(
        self, cmd: Command, conn: Optional[websockets.client.Connect] = None
    ) -> dict:
        ...
        j = json.loads(r)
        if "result" not in j:
            error_msg = json.dumps(cmd.get_websocket_dict(), indent=1)
            eprint(f"Error sending websocket command: {error_msg}")
    ...
    def send_command(self, cmd: Command) -> dict:
        ...
        return self._send_command_line_command(cmd.cmd_id, *cmd.get_command_line_list())
    ...
    def send_subscribe_command(
        ...
        if cmd.websocket is not None:
            ...
            cmd.websocket = (
                ws  # must be set after the _send_websock_command or will unsubscribe
            )

            async def subscribe_callback(
                ws: websockets.client.Connect, cb: Callable[[dict], None]
            ):
                while True:
                    r = await ws.recv()
                    d = json.loads(r)
    ...
    def set_log_level(self, severity: str, *, partition: Optional[str] = None):
        ...
        return self.send_command(LogLevel(severity, partition=partition))
